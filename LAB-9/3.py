...
#TASK-3
def out_degree_matrix(graph):
  max_degree = -1
  for v in range(graph.size):
    degree = graph.get_out_degree(v)
    if degree > max_degree:
      max_degree = degree
  return max_degree

def out_degree_list(graph):
  max_degree = -1
  for v in range(graph.size):
    degree = graph.get_out_degree(v)
    if degree > max_degree:
...
def out_weight_sum_matrix(graph):
  max_sum = -1
  for v in range(graph.size):
    weight_sum = graph.get_out_weight_sum(v)
    if weight_sum > max_sum:
      max_sum = weight_sum
  return max_sum

def out_weight_sum_list(graph):
  max_sum = -1
  for v in range(graph.size):
    weight_sum = graph.get_out_weight_sum(v)
    if weight_sum > max_sum:
...
print("Max Out Degree (Matrix):", out_degree_matrix(directed_matrix))  # Expected: 3
print("Max Out Degree (List):", out_degree_list(directed_list))      # Expected: 3
print("Max Out Weight Sum (Matrix):", out_weight_sum_matrix(directed_matrix))  # Expected: 12 (vertex 2)
print("Max Out Weight Sum (List):", out_weight_sum_list(directed_list))      # Expected: 12
