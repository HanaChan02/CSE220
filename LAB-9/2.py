...
#TASK_2
def weight_sum_matrix(graph):
  max_sum = -1
  for v in range(graph.size):
    weight_sum = graph.get_weight_sum(v)
    if weight_sum > max_sum:
      max_sum = weight_sum
  return max_sum

def weight_sum_list(graph):
  max_sum = -1
  for v in range(graph.size):
    w_sum = graph.get_weight_sum(v)
    if w_sum > max_sum:
      max_sum = w_sum
  return max_sum

  # Test Task 2
print("\nTask 2 - Max Weight Sum:")
print("Matrix:", weight_sum_matrix(matrix_graph))  # Expected: 20 (vertex 2)
print("List:", weight_sum_list(list_graph))       # Expected: 20
